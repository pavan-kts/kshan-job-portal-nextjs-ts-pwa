  ...
  return (
    <div className="flex flex-col items-center w-full py-12 bg-white">
      <div className="w-full max-w-7xl px-6">
        <h2 className="text-2xl font-semibold mb-8">Search by Category</h2>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-5">
          {categories.map((category, index) => (
            <Link key={index} href={category.link || '#'} passHref>
              <div className="flex flex-col items-center h-full p-6 bg-[#DEEFF9] rounded-lg shadow-md hover:shadow-xl transition-shadow">
                ...
                <p className="mt-2 text-gray-600">{category.jobs} Jobs</p>
              </div>
            </Link>
          ))}
        </div>
